...
const create = (data) => {
  const queryString = 'INSERT INTO products (name, categoryid, popularity) VALUES ($1, $2, $3)';
...
const update = (id, data) => {
  const params = Object.values(data);
  const queryString = `UPDATE products SET name = '${params[0]}', categoryid = '${params[1]}', popularity = '${params[2]}' \
  WHERE id = ${id}`;
  client.query(queryString, (err, results) => {
    if (err) {
      return console.error(err);
